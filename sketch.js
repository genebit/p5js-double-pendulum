...
function draw() {
	...
	translate(xPosition, windowHeight / 2);
	stroke(26);
	strokeWeight(6);
	...
	// ANGULAR VELOCITY A
	let angleA_numerator =
		-gravity * (2 * ballA.mass + ballB.mass) * sin(ballA.angle) -
		ballB.mass * gravity * sin(ballA.angle - 2 * ballB.angle) -
		2 *
			sin(ballA.angle - ballB.angle) *
			ballB.mass *
			(ballB.angularVelocity * ballB.angularVelocity * ballB.length +
				ballA.angularVelocity *
					ballA.angularVelocity *
					ballA.length *
					cos(ballA.angle - ballB.angle));

	let angleA_denominator =
		ballA.length *
		(2 * ballA.mass +
			ballB.mass -
			ballB.mass * cos(2 * ballA.angle - 2 * ballB.angle));
	let angularAcceleration1 = angleA_numerator / angleA_denominator;

	// ANGULAR VELOCITY B
	let angle2_numerator =
		2 *
		sin(ballA.angle - ballB.angle) *
		(ballA.angularVelocity *
			ballA.angularVelocity *
			...
			gravity * (ballA.mass + ballB.mass) * cos(ballA.angle) +
